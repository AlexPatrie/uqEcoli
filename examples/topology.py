...
def _():
    ...
    pair2 = Pair(a=Point(*np.random.random(2,)), b=Point(*np.random.random(2,)))
...
def _(Pair, Point, dc, go, np):
    ...
    class ClosedLoop:
                ...
                for point, label in zip(
                    ...
                    fig.add_trace(go.Scatter(
                        ...
                        marker=dict(color=color, size=12,
                                    symbol='circle' if label != 'm' else 'diamond'),
                        showlegend=False,
                    ))
        ...
        def get_random_pair_combinations(self, n: int, seed: int = 42) -> list[tuple[Pair, Pair]]:
            ...
            pairs = self.get_pairs()
            return [
                (rng.choice(pairs), rng.choice(pairs))
                for _ in range(n)
            ]
        ...
        def verify_inscribed_rectangle(self, p1: Pair, p2: Pair, tol: float = 1e-6) -> bool:
            ...
            d1, d2 = p1.d, p2.d
            return (
                np.allclose(m1, m2, atol=tol) and
                abs(d1 - d2) < tol
            )
        ...
        def get_verified_pair_combinations(self, tol: float = 1e-6) -> list[tuple[Pair, Pair]]:
            return [
                (p1, p2)
                for p1, p2 in self.get_pair_combinations()
                if self.verify_inscribed_rectangle(p1, p2, tol=tol)
            ]
...
@app.cell
def _(loop, np):
    def i_rand(): return np.random.randint(low=0, high=len(loop.points) - 1)

    pair_a, pair_b = list(map(
        lambda pair: loop.get_pair(i_a=i_rand(), i_b=i_rand()),
        list(range(2))
    ))
    return
