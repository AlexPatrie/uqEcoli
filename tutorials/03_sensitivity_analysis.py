...
@app.cell
def __(mo, np, param_space, synthetic_model):
    ...
    **Output:** Mean = {Y.mean():.3f}, Std = {Y.std():.3f}
    """)
    return X, Y, lb, n_samples, rng, ub
...
@app.cell
def __(SensitivityAnalyzer, X, Y, mo, param_space):
...
@app.cell
def __(SobolIndices, mo, np, param_space):
    ...
    **Sum of First-Order:** {sobol_indices.first_order.sum():.3f} (should be ~1.0 for additive model)
    """)
    return sobol_indices, total_var
...
@app.cell
def __(mo, param_space, sobol_indices):
