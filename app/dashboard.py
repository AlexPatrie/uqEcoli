...
def channel_strip(data, header, param_dropdown, render_panel):
    ...
    mo.output.replace(
        mo.vstack([
            render_panel(_header, mo.ui.plotly(_fig))
        ]))
    return
...
def pce_eq_plot(data, param_dropdown, param_sliders, surr_data):
    ...
    _normalized_vals = [(_x[_i] - _bounds[_i, 0]) / (_bounds[_i, 1] - _bounds[_i, 0] + 1e-12) for _i in range(len(_params))]

    _fig.add_trace(go.Bar(
        y=_params, x=_normalized_vals,
        orientation="h",
