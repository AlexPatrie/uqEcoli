...
def f_vecoli():
    sim = EcoliSim.from_file((get_repo_root() / "configs" / "vecoli" / "single_cell.json").__str__())
    sim.build_ecoli()
    return sim.run()
...
def uq(
    ...
    results: PipelineResult = asyncio.run(
        pipeline(
            experiment_ids=experiment_ids,
            sim_base_path=outdir_root,
            simulation_func=f_vecoli,
