...
| `report.py` | `generate_html_report()` — self-contained HTML report with inline SVG charts, interactive PCE explorer (client-side JS) |
| `viz_export.py` | Publication-ready Plotly PDFs + LaTeX Sobol table (`export_all_figures()`) |
...
| `uq quantify` | UQPC Steps 4-5: fit PCE surrogates, compute Sobol indices (all 4 strategies). Auto-generates `report.html` |
| `uq report` | Self-contained HTML report with interactive PCE explorer (auto-generated by quantify, also standalone) |
...
├── report.html                  # Self-contained HTML report with interactive PCE explorer
├── manifest.json                # Reproducibility manifest (git SHAs, packages, CLI command, parameter specs)
